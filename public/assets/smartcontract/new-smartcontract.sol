...
contract SupplyChain {
    ...
    function newItem(string memory _text, string memory _date) public returns (bool) {
        Product memory newItem = Product({creator: msg.sender, totalStates: 0, totalValue: 0, productName: _text, productId: items, date: _date});
    ...
    function addValue(uint _productId, uint value) public {
        require(_productId <= items);
        allProducts[_productId].totalValue += value;
    }
    ...
    function getTotalValue(uint _productId) public view returns (uint) {
        require(_productId <= items);
        return allProducts[_productId].totalValue;
    }
